...
use super::*;
use druid::{lens::Identity, Color, Command, FileDialogOptions, FileInfo, Insets, Target, LifeCycle};
    ...
    Flex::column()
        ...
        .with_child(
            Button::new("New").on_click(move |_ctx, data: &mut AppState, _env| {
                data.bases
                    .push_back(CardItem::new().with_name("new".to_string()));
            }),
        )
}
...
fn item_edit() -> impl Widget<CardItem> {
    let name_lbl = Label::new("Имя: ").align_right().padding(10.);
    let name_txt = TextBox::new()
        .with_placeholder("Card title")
        .lens(CardItemEdit.then(CardItemRecord::name))
        .expand_width();

    let path_bin_lbl = Label::new("Исполняемый файл: ").align_right().padding(10.);
    ...
    let path_rsc_lbl = Label::new("Ресурсы: ").align_right().padding(10.);
    ...
    let btn_cancel = Button::new("Cancel").on_click(|ctx, data: &mut CardItem, env| {
        data.edit = None;
    });

    let btn_save = Button::new("Save").on_click(|ctx, data: &mut CardItem, env| {
...
fn item_read() -> impl Widget<CardItem> {
    ...
    let btn_controls = Either::new(|data: &CardItem, env| data.is_hover,
        Flex::row()
            .with_child(btn_edit)
            .with_spacer(5.)
    ...
    fn event(
        ...
        match event {
            Event::Notification(cmd) if cmd.is(REMOVE_CARD_ITEM) => {
                ...
                data.bases.retain(|item| item.id != id);
            }
            _ => (),
        }
    ...
    fn event(
        ...
        match event {
            Event::Command(cmd) if cmd.is(BIN_PATH_CHOOSE_STARTED) => {
                ...
                println!("is_me: {}, {}", &data.id, self.is_dialog_opened_for_this_item);
            }
            ...
            Event::Command(cmd) if cmd.is(PATHS_UPDATED) => {
                ...
                data.edit.as_mut().unwrap().path_resources = resource_path.to_str().unwrap().to_string();
                ctx.set_handled();
            }
